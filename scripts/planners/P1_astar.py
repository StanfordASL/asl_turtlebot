...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        return x[0] >= self.statespace_lo[0] and \
               x[0] <= self.statespace_hi[0] and \
               x[1] >= self.statespace_lo[1] and \
               x[1] <= self.statespace_hi[1] and \
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        deltas = [-self.resolution, 0.0, self.resolution]
        for x_delta in deltas:
            for y_delta in deltas:
                if x_delta == 0.0 and y_delta == 0.0:
                    continue # avoid counting x as its own neighbor
                neighbor_raw = (x[0]+x_delta, x[1]+y_delta)
                neighbor_snapped = self.snap_to_grid(neighbor_raw)
    ...
    def plot_path(self, fig_num=0):
        ...
        plt.plot(solution_path[:,0],solution_path[:,1], color="red", linewidth=5, label="A* solution path", zorder=10)
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            for x_neigh in neighbors:
                ...
                self.est_cost_through[x_neigh] = \
                    cost_to_arrive_at_neighbor + self.distance(x_neigh, self.x_goal)
